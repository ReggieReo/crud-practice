// package StudentManagementSystem;

//Importing required classes
import java.util.Scanner;

import javax.xml.parsers.ParserConfigurationException;
import javax.xml.transform.TransformerException;

public class Main {

	// Main driver method
	public static void main(String[] args) throws ParserConfigurationException, TransformerException {

	// createing AdminMangaement Object.
	AdminRecordManagement adminList = new AdminRecordManagement();
	// Creating Scanner Object to read input
	Scanner input = new Scanner(System.in);
	// ask user for username and password
	String adminUserName, adminPassword;
	System.out.println("Please enter your username: ");
	adminUserName = input.nextLine();
	System.out.println("Please enter your password: ");
	adminPassword = input.nextLine();
		if (adminList.isAdmin(adminUserName, adminPassword)) {
			// Creating HumanResourceOffice Object.
			StudentRecordManagement hr = new StudentRecordManagement();

			// Creating option integer variable
			int option = 0;

			// Do - While loop
			do {
				menu();
				option = input.nextInt();
				input.nextLine();

				// Switch case
				switch (option) {

				// Case 1
				case 1:

					// Display message
					System.out.print("What is the Student id Number ? ");

					int idNumber = input.nextInt();

					// Display message
					System.out.print("What is the Student contact Number ? ");

					String contactNumber = input.next();
					input.nextLine();

					// Display message
					System.out.print("What is the Student Name ? ");

					String name = input.nextLine();

					// Create record object and pass constructor
					// parameters.
					Student record = new Student(name, idNumber, contactNumber);
					// Call add() record
					hr.add(record);
					System.out.println(record.toString());

					// Break statement used to terminate program
					// from here only once it entered this case
					break;

				// Case 2
				case 2:

					// Display message
					System.out.print("What is the Student id number ? ");
					int rId = input.nextInt();

					// Invoke remove/delete record
					hr.delete(rId);

					break;

				// Case 3
				case 3:

					// Display message
					System.out.print("What is the Student id number? ");

					int rIdNo = input.nextInt();
					hr.update(rIdNo, input);

					break;

				// Case 4
				case 4:

					// Display message
					System.out.print("What is the Student id ? ");
					int bookId = input.nextInt();

					if (!hr.find(bookId)) {
						System.out.println("Student id does not exist\n");
					}

					break;

				// Case 5
				case 5:
					System.out.println(hr);
					break;

				// Case 6
				case 6:
					// Display message
					System.out.print("What is the Admin's username ");
					String username = input.nextLine();

					// Display message
					System.out.print("What is the Admin's password ? ");
					String password = input.nextLine();

					adminList.addAdmin(username, password);
					System.out.println(adminList.toString());

					// Break statement used to terminate program
					// from here only once it entered this case
					break;	
				
				// case 7
				case 7:
					System.out.print("What is the Admin's username ");
					String usernameForDeleting = input.nextLine();	
					// delete the admid with that user name
					adminList.removeAdmin(usernameForDeleting);
					break;
					
				
				case 8:
					// update the admin
					adminList.editAdmin();
					break;
				// Case 

				case 9:
					// search the admin
					Admin foundAdmin = adminList.findAdmin();
					if (foundAdmin != null) {
						System.out.println(foundAdmin.toString());
					} else {
						System.out.println("Admin not found");
					}
					break;

				case 10:
					// display all the admins
					System.out.println(adminList.toString());
					break;
				case 0:

					// Display message
					System.out.println("\nThank you for using the program. Goodbye!\n");
					System.exit(0);

					break;

				// Case 7: Default case
				// If none above case executes
				default:

					// Print statement
					System.out.println("\nInvalid input\n");
					break;
				}
				
			}

			// Checking condition
			while (option != 0);
		}
	}

	// Method 2
	// Menu - Static menu for displaying options
	public static void menu() {

		// Printing statements displaying menu on console
		System.out.println("MENU");
		System.out.println("1: Add Student");
		System.out.println("2: Delete Student");
		System.out.println("3: Update Student");
		System.out.println("4: Search Student");
		System.out.println("5: Display Students");
		System.out.println("6: Add Admin");
		System.out.println("7: Delete Admin");
		System.out.println("8: Update Admin");
		System.out.println("9: Search Admin");
		System.out.println("10: Display Admins");
		System.out.println("0: Exit program");
		System.out.print("Enter your selection : ");
	}

}
